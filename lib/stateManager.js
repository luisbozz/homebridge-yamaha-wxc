module.exports = {
	...
	getState: async function() {
		...
		try {
			...
			const currentInputKey = normalizeInputKey(basicInfo.getCurrentInput())
			const currentInput = this.inputs.find(input => normalizeInputKey(input.key) === currentInputKey)
	...
	set: {
		...
		ActiveIdentifier: async function(identifier, callback) {
			const input = this.inputs.find(sourceInput => sourceInput.identifier === identifier)
			if (!input) {
				this.log(`${this.name} - Unknown input identifier ${identifier}`)
				callback()
				return
			}
...
const normalizeInputKey = function(inputKey) {
	return String(inputKey || '').replace(/[\s_-]/g, '').toLowerCase()
}
